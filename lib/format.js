/**
 * @import {BlockContent, DefinitionContent, InlineCode, ListItem, PhrasingContent, RootContent, Root} from 'mdast'
  ...
  if (field.description) {
    ...
    if (
      ...
      if (first && first.type === 'text') {
        ...
        do {
          code = first.value.charCodeAt(++firstWordBreak)
        } while (!unicodePunctuation(code) && !unicodeWhitespace(code))

        const firstCharacter = first.value.slice(0, 1)
        const restOfWord = first.value.slice(1, firstWordBreak)
        ...
        if (
          ...
          first.value = firstCharacter.toLowerCase() + first.value.slice(1)
        }
      }

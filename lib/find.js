/**
 * @typedef {import('module-exports').Result} Result
  ...
  async function finder(url) {
    ...
    if (!cached) {
      ...
      for (const from of commandLine.fileNames) {
        let to = typescript.getOutputFileNames(
          commandLine,
          from,
    ...
    const sourcePath = cached.toSource.get(filePath) || filePath
    const sourceFile = cached.program.getSourceFile(sourcePath)
    const configFolder = path.dirname(configPath)

    if (!sourceFile) {
      const available = new Set(
        [...cached.toSource.keys(), ...cached.toSource.values()].map((d) =>
          relativePathToFilePath(path.relative(configFolder, d))
        )
      )

      throw new VFileMessage(
        'Could not figure out source file for `' +
          relativePathToFilePath(path.relative(configFolder, sourcePath)) +
          '`; the ' +
